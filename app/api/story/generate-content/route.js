...
export async function POST(req) {
  try {
    const { userId: clerkId, prompt } = await req.json();
    ...
    if (hasCustomKeys) {
      try {
        geminiApiKey = decrypt(user.apiKeys.geminiApiKey);
        console.log("✅ Using user's Gemini API key");
      } catch (error) {
        console.error("Failed to decrypt user's Gemini API key:", error);
        console.log("⚠️ Falling back to system Gemini API key");
      }
    } else {
      // Check quota before allowing generation with system key
      ...
      console.log("Using system Gemini API key");
    }

    // Initialize Gemini AI with the appropriate key
